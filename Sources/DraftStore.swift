...
final class DraftStore {
    ...
    init(root: URL) throws {
        self.root = root
        try fm.createDirectory(at: root.appendingPathComponent("drafts"), withIntermediateDirectories: true)
        ...
        if fm.fileExists(atPath: pointer.path) {
            ...
            guard UUID(uuidString: id) != nil else { throw PaperError("The draft reference is damaged. Existing files have been preserved.") }
            draft = try JSONDecoder().decode(Draft.self, from: Data(contentsOf: root.appendingPathComponent("drafts/\(id)/manifest.json")))
            guard draft.id == id else { throw PaperError("The draft identity does not match its folder.") }
        } else {
    ...
    private func initializeDraft() throws {
        ...
        try fm.createDirectory(at: folder.appendingPathComponent("ingests"), withIntermediateDirectories: true)
    ...
    private func newDraft() throws {
        let previous = draft
        draft = Draft(id: UUID().uuidString, created: Date())
        do { try initializeDraft() } catch { draft = previous; throw error }
    }
    ...
    private func editable() throws {
        guard draft.export == nil else { throw PaperError("A PDF save is pending. Retry Save to finish it before adding pages.") }
    }
    func beginCapture() throws { try editable(); var next = draft; next.interrupted = true; try commit(next) }
    func completeCapture(success: Bool) throws { var next = draft; next.interrupted = !success; try commit(next) }

    @discardableResult func ingest(_ input: URL, dpi: Double = 300, autoCrop: Bool = false) throws -> Int {
        try editable()
        ...
        for index in 0..<CGImageSourceGetCount(source) {
            guard let image = CGImageSourceCreateImageAtIndex(source, index, nil) else { throw PaperError("An incomplete page was received. Earlier pages are saved.") }
            let properties = CGImageSourceCopyPropertiesAtIndex(source, index, nil) as? [CFString: Any]
            let reportedDPI = (properties?[kCGImagePropertyDPIWidth] as? NSNumber)?.doubleValue ?? dpi
            pages.append(StoredPage(id: UUID().uuidString, source: "\(id).image", frame: index, width: image.width, height: image.height, dpi: reportedDPI > 0 ? reportedDPI : dpi))
        }
        try durableWrite(data, to: folder.appendingPathComponent("sources/\(id).image"))
        for index in pages.indices {
            pages[index].cropReviewed = true
            if autoCrop { pages[index].crop = AutoCrop.detect(try image(for: pages[index], cropped: false)) }
        }
        ...
        var next = draft; next.pages += pages; next.ingests.append(id)
        try commit(next)
        return pages.count
    }
    private func reconcile() throws {
        guard draft.export == nil else { return }
        let envelopes = try fm.contentsOfDirectory(at: folder.appendingPathComponent("ingests"), includingPropertiesForKeys: nil)
        ...
        for envelope in envelopes where !next.ingests.contains(envelope.id) {
            ...
            next.pages += envelope.pages; next.ingests.append(envelope.id); recoveredCount += envelope.pages.count
        }
        if recoveredCount > 0 { next.interrupted = true; try commit(next) }
        for page in draft.pages {
            guard fm.fileExists(atPath: folder.appendingPathComponent("sources/\(page.source)").path) else {
    ...
    func image(for page: StoredPage, cropped: Bool = true) throws -> CGImage {
        ...
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
              let image = CGImageSourceCreateThumbnailAtIndex(source, page.frame, [
                kCGImageSourceCreateThumbnailFromImageAlways: true,
                kCGImageSourceCreateThumbnailWithTransform: true,
                kCGImageSourceThumbnailMaxPixelSize: max(page.width, page.height)
              ] as CFDictionary) else { throw PaperError("A saved page could not be opened.") }
    ...
    func setAutoCrop(_ id: String, enabled: Bool) throws {
        ...
        try update(id) { $0.crop = crop; $0.cropReviewed = true }
    }
    func cropUnreviewedPages() throws {
        guard draft.export == nil else { return }
        try editable(); var next = draft
    ...
    private func update(_ id: String, change: (inout StoredPage) -> Void) throws {
        ...
        change(&next.pages[index]); try commit(next)
    }
    func restoreLastRemoved() throws {
        try editable(); var next = draft
        guard let index = next.pages.lastIndex(where: { $0.removed }) else { return }
        next.pages[index].removed = false; try commit(next)
    }
    func move(_ id: String, by offset: Int) throws {
        try editable(); let visible = visiblePages
        guard let current = visible.firstIndex(where: { $0.id == id }), visible.indices.contains(current + offset),
              let from = draft.pages.firstIndex(where: { $0.id == id }),
              let to = draft.pages.firstIndex(where: { $0.id == visible[current + offset].id }) else { return }
        var next = draft; next.pages.swapAt(from, to); try commit(next)
    }
    ...
    func preview(_ page: StoredPage) throws -> PDFDocument {
        let doc = PDFDocument(); doc.insert(try pdfPage(page), at: 0); return doc
    }
    private func pdfPage(_ page: StoredPage) throws -> PDFPage {
        let image = try image(for: page)
        let size = NSSize(width: Double(image.width) * 72 / page.dpi, height: Double(image.height) * 72 / page.dpi)
        guard let result = PDFPage(image: NSImage(cgImage: image, size: size)) else { throw PaperError("Could not render a saved page.") }
        result.rotation = page.rotation; return result
    }
    ...
    func export(to destination: URL) throws -> URL {
        ...
        if draft.export == nil {
            ...
            guard let data = doc.dataRepresentation(), let check = PDFDocument(data: data), check.pageCount == visiblePages.count else { throw PaperError("PDF verification failed. Your pages are still saved.") }
            ...
            var next = draft
            next.export = ExportRecord(destination: destination.appendingPathComponent(name), sha256: hash(data), published: false)
            try commit(next)
        }
        ...
        guard hash(data) == record.sha256 else { throw PaperError("The pending PDF has changed. Source pages are preserved.") }
        if fm.fileExists(atPath: record.destination.path) {
            guard hash(try Data(contentsOf: record.destination)) == record.sha256 else { throw PaperError("A different file already exists at the output name. Nothing was overwritten.") }
        } else {
            ...
            let fd = open(parent.path, O_RDONLY); if fd >= 0 { _ = fsync(fd); close(fd) }
        }
        record.published = true; var next = draft; next.export = record; next.interrupted = false
        try commit(next)
        try newDraft()
