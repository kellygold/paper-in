...
let root = FileManager().temporaryDirectory.appendingPathComponent("PaperIn-ConnectionTests-\(UUID().uuidString)")
defer { try? FileManager().removeItem(at: root) }
let defaultsName = "PaperIn.ConnectionTests.\(UUID().uuidString)"
let defaults = UserDefaults(suiteName: defaultsName)!
defer { defaults.removePersistentDomain(forName: defaultsName) }
let browser = FakeBrowser(), device = FakeScanner()
let controller = ScannerController(staging: root.appendingPathComponent("transfers"), defaults: defaults, browserFactory: { browser })
...
check(device.opens == 1 && !controller.listening, "Device flapping caused repeated connection attempts")
