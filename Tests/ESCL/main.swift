...
@main enum Tests {
  static func main() async throws {
    ...
    config.protocolClasses = [MockUSB.self]
    let client = ESCLClient(
      base: URL(string: "http://localhost:12345/eSCL")!, session: URLSession(configuration: config))
    MockUSB.responses = [(409, [:], Data())]
    do {
      _ = try await client.create(settings: DS940Profile().settings(options: ScanOptions()))
      fatalError("409 accepted")
    ...
    do {
      _ = try await client.create(settings: DS940Profile().settings(options: ScanOptions()))
      fatalError("Foreign job accepted")
    } catch {}
    precondition(MockUSB.methods == ["POST", "POST"])
    print("PASS rejection and foreign job URL do not trigger page retrieval")

    let bitmap = NSBitmapImageRep(
      bitmapDataPlanes: nil, pixelsWide: 40, pixelsHigh: 60, bitsPerSample: 8, samplesPerPixel: 3,
    ...
    for index in 0..<2 {
      MockUSB.responses = [
        (201, ["Location": "/eSCL/ScanJobs/owned"], Data()),
        (200, ["Content-Type": "image/jpeg"], jpeg), (200, [:], Data()),
      ]
      try store.beginCapture()
      let job = try await client.create(settings: DS940Profile().settings(options: ScanOptions()))
      let file = root.appendingPathComponent("page-\(index).jpg")
      try await client.page(job, to: file)
      try store.ingest(file)
      try await client.close(job)
    ...
    precondition(
      Array(MockUSB.methods.suffix(6)) == ["POST", "GET", "DELETE", "POST", "GET", "DELETE"])
    ...
    do {
      try await client.page(
        URL(string: "http://localhost:12345/eSCL/ScanJobs/owned")!,
        to: root.appendingPathComponent("bad.jpg"))
      fatalError("Invalid image accepted")
    } catch {}
    print("PASS incomplete image rejected and raw response retained")
    let xml = try ScanXML.read(
      Data(
        "<scan:ScannerStatus xmlns:scan='urn:scan' xmlns:pwg='urn:pwg'><pwg:State>Idle</pwg:State><scan:AdfState>ScannerAdfLoaded</scan:AdfState></scan:ScannerStatus>"
          .utf8))
