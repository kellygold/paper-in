...
final class ScannerController: NSObject, ObservableObject, ICDeviceBrowserDelegate,
  ICScannerDeviceDelegate
{
  ...
  init(
    staging: URL, defaults: UserDefaults = UserDefaults(),
    browserFactory: @escaping () -> ICDeviceBrowser = { ICDeviceBrowser() }
  ) {
  ...
  func connect() {
    ...
    listening = true
    message = "Looking for your scanner…"
    let b = browserFactory()
    browser = b
    b.delegate = self
  ...
  func pause() {
    ...
    if busy {
      scanner?.cancelScan()
      finish(false, "Scanning stopped. Completed pages are saved.")
    }
    connected = false
    opening = false
    selecting = false
  ...
  func retry() {
    ...
    if let scanner, !opening, !connected { open(scanner) } else if !listening { connect() }
  }
  ...
  private func open(_ device: ICScannerDevice) {
    ...
    opening = true
    connected = false
    scannerName = device.name ?? "Brother DS-940DW"
    message = "Connecting…"
    connectionTimer?.invalidate()
    connectionTimer = Timer.scheduledTimer(withTimeInterval: 20, repeats: false) {
      [weak self, weak device] _ in
      guard let self, let device, self.scanner === device, !self.connected else { return }
      self.pause()
      self.message = "Connection timed out. Your pages are saved. Try Connect again."
    }
  ...
  func deviceBrowser(_ browser: ICDeviceBrowser, didAdd device: ICDevice, moreComing: Bool) {
    ...
    if let saved {
      guard identity(device) == saved else { return }
    } else {
      guard (device.name ?? "").localizedCaseInsensitiveContains("DS-940") else { return }
    }
  ...
  private func removed(_ device: ICDevice) {
    ...
    pause()
    message =
      "Connection dropped. Your pages are saved. Check the USB connection, then click Connect."
  }
  func device(_ device: ICDevice, didOpenSessionWithError error: Error?) {
    ...
    if let error {
      pause()
      message =
        "Couldn’t connect: \(error.localizedDescription). Click Connect when you’re ready to retry."
      return
    }
  ...
  private func selectFeeder() {
    ...
    guard
      scanner.availableFunctionalUnitTypes.contains(
        NSNumber(value: ICScannerFunctionalUnitType.documentFeeder.rawValue))
    else {
      message = "This connection does not expose a document feeder."
      connectionTimer?.invalidate()
      return
    }
    selecting = true
  ...
  func scannerDevice(
    _ scanner: ICScannerDevice, didSelect functionalUnit: ICScannerFunctionalUnit, error: Error?
  ) {
    guard self.scanner === scanner, listening, scanner.hasOpenSession else { return }
    selecting = false
    if let error {
      message = "Couldn’t prepare scanner: \(error.localizedDescription)"
      connectionTimer?.invalidate()
      return
    }
    ...
    diagnostics.event(
      ...
      [
        ...
        "scan_area": NSStringFromRect(feeder.scanArea),
        "document_size": NSStringFromSize(feeder.documentSize),
        "physical_size": NSStringFromSize(feeder.physicalSize),
        "measurement_unit": feeder.measurementUnit.rawValue,
      ])
    supportsDuplex = feeder.supportsDuplexScanning
    connected = true
    connectionTimer?.invalidate()
    message = "Ready — insert a sheet and press Scan"
    if pendingButton {
      pendingButton = false
      scan()
    }
  ...
  func scan() {
    guard listening, connected, !busy, let scanner,
      let feeder = scanner.selectedFunctionalUnit as? ICScannerFunctionalUnitDocumentFeeder
    else { return }
    do {
      guard feeder.supportedResolutions.contains(300), feeder.supportedBitDepths.contains(8),
        feeder.supportedDocumentTypes.contains(Int(ICScannerDocumentType.typeA4.rawValue))
      else {
        throw PaperError(
          "This connection doesn’t expose the A4 / 300 dpi preset. No scan was started.")
      }
      guard !duplex || feeder.supportsDuplexScanning else {
        throw PaperError("Two-sided scanning isn’t available on this connection.")
      }
      ...
      scanner.downloadsDirectory = dir
      ...
      jobTimer = Timer.scheduledTimer(withTimeInterval: 120, repeats: false) { [weak self] _ in
        ...
        self.pause()
        self.message =
          "Scanner stopped responding. Completed pages are saved. Reconnect before trying the sheet again."
      }
      diagnostics.event(
        "scan_requested",
        [
          "job": dir.lastPathComponent, "dpi": feeder.resolution, "duplex": duplex,
          "reported_paper_loaded": feeder.documentLoaded,
          "scan_area": NSStringFromRect(feeder.scanArea),
          "document_size": NSStringFromSize(feeder.documentSize),
          "measurement_unit": feeder.measurementUnit.rawValue,
          "pixel_type": feeder.pixelDataType.rawValue,
          "bit_depth": feeder.bitDepth.rawValue, "image_format": scanner.documentUTI,
        ])
      scanner.requestScan()
    } catch {
      diagnostics.event("scan_not_started", error: error)
  ...
  func scannerDevice(_ scanner: ICScannerDevice, didScanTo url: URL) {
    ...
    guard canonical.deletingLastPathComponent() == dir.resolvingSymlinksInPath().standardizedFileURL
    else {
    ...
    do {
      diagnostics.event(
        "page_received", ["job": dir.lastPathComponent, "file": url.lastPathComponent])
      try onPage?(url, Double(scanner.selectedFunctionalUnit.resolution))
      delivered.insert(canonical.path)
      diagnostics.event(
        "page_saved", ["job": dir.lastPathComponent, "received_files": delivered.count])
    } catch {
      diagnostics.event("page_save_failed", error: error)
      captureFailed = true
      scanner.cancelScan()
      message =
        "Couldn’t save this page: \(error.localizedDescription). Earlier pages are preserved."
    }
  }
  func scannerDevice(_ scanner: ICScannerDevice, didCompleteScanWithError error: Error?) {
    guard self.scanner === scanner, busy else { return }
    diagnostics.event(
      "scan_completed", ["received_files": delivered.count, "save_failed": captureFailed],
      error: error)
    if captureFailed {
      finish(false, message)
    } else if let error {
      finish(false, "Scan interrupted: \(error.localizedDescription). Completed pages are saved.")
    } else if delivered.isEmpty {
      finish(
        false,
        "The scan ended without delivering an image. Your saved pages are preserved; the connection log has the scan details."
      )
    } else {
      finish(true, nil)
    }
