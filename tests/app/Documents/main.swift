...
func fixture(_ name: String, frames: Int = 1, orientation: Int = 1) throws -> URL {
  ...
  guard
    let destination = CGImageDestinationCreateWithURL(
      url as CFURL, "public.tiff" as CFString, frames, nil)
  else { throw PaperError("Fixture destination") }
  for index in 0..<frames {
    let context = CGContext(
      data: nil, width: 96, height: 160, bitsPerComponent: 8, bytesPerRow: 384,
...
try test("Append across restart, preserve original bytes and two-sided order") {
  ...
  try expect(
    try Data(contentsOf: single)
      == Data(contentsOf: store.folder.appendingPathComponent("sources/\(first.source)")),
    "Original image bytes changed")
  ...
  try expect(
    abs(pdf.page(at: 0)!.bounds(for: .mediaBox).width - 96) < 1, "PDF physical width is wrong")
...
try test("Interrupted capture survives restart") {
  ...
  try expect(
    recovered.draft.interrupted && recovered.visiblePages.count == 1,
    "Interrupted draft wasn't restored")
}
try test("Recover page received before manifest write failure exactly once") {
  ...
  let store = try DraftStore(root: root)
  store.beforeWrite = {
    if $0.lastPathComponent == "manifest.json" { throw PaperError("Simulated disk full") }
  }
  do {
    try store.ingest(single)
    throw PaperError("Failure wasn't surfaced")
  } catch { try expect(store.visiblePages.isEmpty, "Reported saved before commit") }
  let recovered = try DraftStore(root: root)
  try expect(
    recovered.visiblePages.count == 1 && recovered.recoveredCount == 1,
    "Completed page wasn't recovered")
...
try test("Rotation, reordering and reversible removal survive restart") {
  ...
  var store = try DraftStore(root: root)
  try store.ingest(duplex)
  let first = store.visiblePages[0].id
  let second = store.visiblePages[1].id
  try store.rotate(first)
  try store.move(first, by: 1)
  ...
  try expect(
    store.visiblePages.map(\.id) == [second, first], "Restore or reorder changed sequence")
...
try test("Export interrupted after publication retries without duplicate PDFs") {
  let root = base.appendingPathComponent("retry-export")
  let output = base.appendingPathComponent("output-retry")
  let store = try DraftStore(root: root)
  try store.ingest(single)
  store.beforeWrite = { url in
    if url.lastPathComponent == "manifest.json", store.draft.export != nil {
      throw PaperError("Simulated interruption after PDF publication")
    }
  }
  do {
    _ = try store.export(to: output)
    throw PaperError("Failure wasn't surfaced")
  } catch {}
  ...
  try expect(
    try fm.contentsOfDirectory(atPath: output.path).filter { $0.hasSuffix(".pdf") }.count == 1,
    "Retry duplicated PDF")
...
try test("Output failure retains pages; conflicting file is never overwritten") {
  let root = base.appendingPathComponent("collision")
  let output = base.appendingPathComponent("output-collision")
  let store = try DraftStore(root: root)
  try store.ingest(single)
  store.beforeWrite = { url in
    if url.path.hasPrefix(output.path) { throw PaperError("Output disk full") }
  }
  do { _ = try store.export(to: output) } catch {}
  try expect(
    store.visiblePages.count == 1 && store.draft.export != nil, "Output failure lost draft")
  let target = store.draft.export!.destination
  try fm.createDirectory(at: output, withIntermediateDirectories: true)
  let other = Data("Existing unrelated file".utf8)
...
try test("Organizer may move exported PDF; new document stays independent") {
  let root = base.appendingPathComponent("organizer")
  let output = base.appendingPathComponent("output-organizer")
  let store = try DraftStore(root: root)
  try store.ingest(single)
  let first = try store.export(to: output)
  let moved = base.appendingPathComponent("filed.pdf")
  try fm.moveItem(at: first, to: moved)
  try store.ingest(duplex)
  let second = try store.export(to: output)
  try expect(!fm.fileExists(atPath: first.path), "Old output recreated")
  try expect(
    PDFDocument(url: moved)!.pageCount == 1 && PDFDocument(url: second)!.pageCount == 2,
    "Documents contaminated")
}
try test("Unreadable input is rejected without changing previous pages") {
  let store = try DraftStore(root: base.appendingPathComponent("bad-input"))
  try store.ingest(single)
  let bad = base.appendingPathComponent("bad.image")
...
try test("Scanner image orientation is honored without altering source bytes") {
  ...
  try expect(
    abs(bounds.width - 160) < 1 && abs(bounds.height - 96) < 1, "EXIF orientation was ignored")
}

try test("Duplex sheets retain pairing through removal, reordering and interrupted capture") {
  let root = base.appendingPathComponent("sheet-pairs")
  var store = try DraftStore(root: root)
  try store.beginCapture(expectedSides: 2)
  try store.ingest(single)
  try store.ingest(single)
  try store.completeCapture(success: true)
  let front = store.visiblePages[0]
  let back = store.visiblePages[1]
  try store.beginCapture(expectedSides: 2)
  try store.ingest(single)
  try store.completeCapture(success: false)
  try store.remove(back.id)
  try store.move(front.id, by: 1)
  store = try DraftStore(root: root)
  let groups = SheetGroup.make(store.draft.pages)
  try expect(groups.count == 2, "Wrong sheet count")
  let first = groups.first { $0.id == front.sheetID }!
  try expect(
    first.page(side: 1)?.removed == true && first.page(side: 0)?.id == front.id,
    "Pair shifted after removal")
  let incomplete = groups.first { $0.id != front.sheetID }!
  try expect(
    incomplete.expectedSides == 2 && incomplete.page(side: 1) == nil, "Missing back not preserved")
}
try test("Legacy pages remain unpaired and filing intent survives export") {
  let root = base.appendingPathComponent("filing-intent")
  let store = try DraftStore(root: root)
  try store.ingest(duplex)
  try expect(SheetGroup.make(store.draft.pages).count == 2, "Legacy sides were guessed")
  let archive = store.folder
  let intent = ExportFilingIntent(root: base.path, settings: FilingSettings())
  _ = try store.export(to: base.appendingPathComponent("intent-output"), filing: intent)
  let saved = try JSONDecoder().decode(
    Draft.self, from: Data(contentsOf: archive.appendingPathComponent("manifest.json")))
  try expect(
    saved.export?.published == true && saved.export?.filing?.root == base.path,
    "Filing handoff not durable")
}
try test("Starting a new capture reconciles a completed page after manifest failure") {
  let store = try DraftStore(root: base.appendingPathComponent("same-process-recovery"))
  try store.beginCapture(expectedSides: 2)
  store.beforeWrite = {
    if $0.lastPathComponent == "manifest.json" { throw PaperError("Simulated failure") }
  }
  do { try store.ingest(single) } catch {}
  store.beforeWrite = nil
  try store.beginCapture(expectedSides: 1)
  try expect(store.visiblePages.count == 1, "Complete prior image was left orphaned")
  try store.ingest(single)
  try expect(
